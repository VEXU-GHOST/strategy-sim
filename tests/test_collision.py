...
def test_two_robots_move_to_same_cell() -> None:
    ...
    result = resolve_moves(
        current, proposed, blocked_cells=set(), robot_radius_cells=ROBOT_RADIUS
    )
...
def test_move_into_blocked_cell_reverts() -> None:
    ...
    result = resolve_moves(
        current, proposed, blocked_cells=blocked, robot_radius_cells=ROBOT_RADIUS
    )
